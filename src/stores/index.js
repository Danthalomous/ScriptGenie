...
export default createStore({
  ...
  mutations: {
    ...
    pushToOrganizations(state, organization) {
  ...
  actions: {
    async login({ commit }, token) {
      try {
        ...
        await this.dispatch('fetchOrganizations');
      } catch (error) {
        console.log(error);
      }
    },
    async fetchOrganizations({ commit, getters }) {
      try {
        ...
        const organizationsResponse = await axios.get('https://localhost:7273/profile/fetchAll/organizations?userID=' + getters.userID);
